...
#include <debug-draw/debug_draw.hpp>     // Debug Draw API. Notice that we need the DEBUG_DRAW_IMPLEMENTATION macro here!
...
int main(int argc, char *argv[])
{
    ...
    // setup subsystems
    sf::RenderWindow window(sf::VideoMode(1280, 720), "UsagiEditor", sf::Style::Default, settings);
        ...
        while(window.pollEvent(event))
        {
            ...
            switch(event.type)
            {
                ...
                case sf::Event::MouseMoved:
                {
                    ...
                    mouse.deltaX = event.mouseMove.x - mouse.lastPosX;
                    mouse.deltaY = event.mouseMove.y - mouse.lastPosY;
                    mouse.lastPosX = event.mouseMove.x;
                    mouse.lastPosY = event.mouseMove.y;
                ...
                {
                    if(event.key.code == sf::Keyboard::A || event.key.code == sf::Keyboard::Left) { keys.aDown = true; }
                    else if(event.key.code == sf::Keyboard::D || event.key.code == sf::Keyboard::Right) { keys.dDown = true; }
                    else if(event.key.code == sf::Keyboard::W || event.key.code == sf::Keyboard::Up) { keys.wDown = true; }
                    else if(event.key.code == sf::Keyboard::S || event.key.code == sf::Keyboard::Down) { keys.sDown = true; }
                    // Toggleable flags:
                ...
                {
                    if(event.key.code == sf::Keyboard::A || event.key.code == sf::Keyboard::Left) { keys.aDown = false; }
                    else if(event.key.code == sf::Keyboard::D || event.key.code == sf::Keyboard::Right) { keys.dDown = false; }
                    else if(event.key.code == sf::Keyboard::W || event.key.code == sf::Keyboard::Up) { keys.wDown = false; }
                    else if(event.key.code == sf::Keyboard::S || event.key.code == sf::Keyboard::Down) { keys.sDown = false; }
                    break;
                }
